<title>Diagnosa - Printer 1.0</title>
<?php
switch ($_GET['act']) {
...
// --------------------- END -------------------------
	...
	<h2 class='text text-primary'>Hasil Diagnosis &nbsp;&nbsp;<button id='print' onClick='window.print();' data-toggle='tooltip' data-placement='right' title='Klik tombol ini untuk mencetak hasil diagnosa'><i class='fa fa-print'></i> Cetak</button> </h2>
	          <hr><table class='table table-bordered table-striped diagnosa1'> 
          <th width=8%>No</th>
          <th width=10%>Kode</th>
      ...
      foreach ($argejala as $key => $value) {
        ...
        $ig++;
        $gejala = $key;
        $sql4 = mysql_query("SELECT * FROM gejala1 where kode_gejala = '$key'");
        $r4 = mysql_fetch_array($sql4);
        echo '<tr><td>' . $ig . '</td>';
        echo '<td>G' . str_pad($r4[kode_gejala], STR_PAD_LEFT) . '</td>';
      ...
      echo "<div class='callout callout-default'>Jenis kerusakan yang diderita adalah <b><h3 class='text text-success'>" . $nmpkt[1] . "</b> / " . round(($vlpkt[1])*100, 2) . " % (" . ($vlpkt[1]). ")<br></h3>";
	 ...
	 <div class='alert alert-success alert-dismissible'>
                ...
                Silahkan memilih gejala sesuai dengan kondisi printer anda, anda dapat memilih kepastian kondisi printer dari pasti tidak sampai pasti ya, jika sudah tekan tombol proses (<i class='fa fa-search-plus'></i>)  di bawah untuk melihat hasil.
              </div>
		<form name=text_form method=POST action='diagnosa1' >
      ...
      $sql3 = mysql_query("SELECT * FROM gejala1 order by kode_gejala");
      $i = 0;
