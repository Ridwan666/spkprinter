...
// --------------------- END -------------------------
	...
	<h2 class='text text-primary'>Hasil Diagnosis &nbsp;&nbsp;<button id='print' onClick='window.print();' data-toggle='tooltip' data-placement='right' title='Klik tombol ini untuk mencetak hasil diagnosa'><i class='fa fa-print'></i> Cetak</button> </h2>
	          <hr><table class='table table-bordered table-striped diagnosa2'> 
          <th width=8%>No</th>
          <th width=10%>Kode</th>
      ...
      foreach ($argejala as $key => $value) {
        ...
        $ig++;
        $gejala = $key;
        $sql4 = mysql_query("SELECT * FROM gejala2 where kode_gejala = '$key'");
      ...
      echo "<div class='callout callout-default'>Jenis penyakit yang diderita adalah <b><h3 class='text text-success'>" . $nmpkt[1] . "</b> / " . round(($vlpkt[1]+0.48)*100, 2) . " % (" . ($vlpkt[1]+0.48). ")<br></h3>";
	 ...
	 <div class='alert alert-success alert-dismissible'>
		...
		<form name=text_form method=POST action='diagnosa2' >
      ...
      $sql3 = mysql_query("SELECT * FROM gejala2 order by kode_gejala");
      $i = 0;
